  ...
  for(let day=1; day<=daysInMonth; day++){
    ...
    cells += `
      <div class="cal-day ${perfect?'perfect':''} ${isToday?'today':''} ${isSelected?'selected':''}" data-iso="${iso}">
        ...
        <div class="dots">
          ${habitsConfig.map(h => `<span style="background-color:${data[h.key] ? h.color : 'var(--border)'}"></span>`).join('')}
        </div>
      </div>`;
  }
...
function openDayEditor(iso){
  ...
  document.getElementById('dayEditorToggles').innerHTML = habitsConfig.map(h => `
    <button class="toggle-btn ${data[h.key] ? 'on':''}" data-key="${h.key}" style="--btn-color:${h.color}" aria-pressed="${data[h.key] ? 'true':'false'}">
      ${h.icon} ${h.label}
    </button>
  `).join('');
